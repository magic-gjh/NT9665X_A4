...
GPIO_INIT_OBJ uiGPIOMapInitTab[] = {
    ...
    //ACC Det
    {  GPIO_ACC_DET,      GPIO_DIR_INPUT,    PAD_PULLUP,   PAD_ACC_DET      },       
    //EDOG EN
    {  GPIO_EDOG_POWER,           GPIO_DIR_OUTPUT,      GPIO_SET_OUTPUT_LOW,           PAD_EDOG_POWER},  
    ...
    {  GPIO_REVERSE_GEAR_DET,            GPIO_DIR_INPUT,     PAD_PULLUP,           PAD_REVERSE_GEAR_DET},	
#if 0
    //KEY
...
BOOL GPIOMap_DetDC(void)
{
	return gpio_getPin(GPIO_DC_DET);
}

BOOL GPIOMap_IsEdogPowerOK(void)
{
	return gpio_getPin(GPIO_EDOG_POWER);
}

void GPIOMap_TurnOnEdog(BOOL En)
{
	if(En==TRUE)
	gpio_setPin(GPIO_EDOG_POWER);
	else
	gpio_clearPin(GPIO_EDOG_POWER);		
}
