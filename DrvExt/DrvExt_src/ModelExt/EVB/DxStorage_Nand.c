...
UINT32 DrvNANDGetcaps2(UINT32 CapID, UINT32 Param1); // Get Capability Flag (Base on interface version)
...
{
    DXFLAG_SIGN,
    DX_CLASS_STORAGE_EXT|DX_TYPE_NAND2,
    STORAGE_VER,
    "Storage_Nand2",
    0,0,0,0,
    DrvNANDGetcaps2,
...
UINT32 DrvNANDGetcaps2(UINT32 CapID, UINT32 Param1) // Get Capability Flag (Base on interface version)
{
    ...
    case STORAGE_CAPS_HANDLE:
        ...
        v = (UINT32)ramdsk_getStorageObject();
#else
        v = (UINT32)nand_getStorageObject(STRG_OBJ_FAT2);
#endif
        break;
    default:
        break;
