...
#endif
    uiKeyCode |= DrvKey_DetGsensorEvent();
...
/**
  Detect IR key is pressed or not from Car Touch Panel.

  Detect IR key is pressed or not from Car Touch Panel.
  Return X and Y coordinates for Car Touch Panel.
...
/**
  Detect G-sensor is trigger or not.

  Detect G-sensor is trigger or not.
  Return pseudo key pressed status (refer to KeyDef.h)
