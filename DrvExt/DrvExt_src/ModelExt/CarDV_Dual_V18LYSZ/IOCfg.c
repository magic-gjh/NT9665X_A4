...
GPIO_INIT_OBJ uiGPIOMapInitTab[] = {
    ...
    {  GPIO_12VDC_DET,            GPIO_DIR_INPUT,     PAD_PULLUP,           PAD_12VDC_DET          },
...
{
    return (gpio_getPin(GPIO_12VDC_DET)? FALSE : TRUE);
}

/**
    DGPIO1  Low ,ACC in .else DGPIO1 hi ,no ACC.

*/
BOOL GPIOMap_DetACCIn(void)
{
    //false is ACC out,need power off
    return FALSE;//(gpio_getPin(GPIO_DET_ACC)? FALSE : TRUE);
}
