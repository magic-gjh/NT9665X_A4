...
GPIO_INIT_OBJ uiGPIOMapInitTab[] = {
    ...
    //White Light
    {  GPIO_WHITE_LIGHT,      GPIO_DIR_OUTPUT,    GPIO_SET_OUTPUT_HI,   PAD_WHITE_LIGHT      },        
    //BG LED
    //{  GPIO_BG_LED,      GPIO_DIR_OUTPUT,    GPIO_SET_OUTPUT_LOW,   PAD_BG_LED},      
    //GPS
...
void GPIOMap_TurnOnBGLed(void)
{
	gpio_setPin(GPIO_BG_LED);
}
void GPIOMap_TurnOffBGLed(void)
{
	gpio_clearPin(GPIO_BG_LED);
}

BOOL GPIOMap_IsBGLedOn(void)
{
	return gpio_getPin(GPIO_BG_LED);
}

void GPIOMap_TurnOnGPSPower(void)
{
	gpio_setPin(GPIO_GPS_PWR);
}
