...
static ER dsi_LPWriteCmd(DSI_LANESEL uiDataLane, UINT32 uiDCSCmd, UINT8 * param, UINT32 paramNo, DSI_PACKET_TYPE pktType, BOOL bEOT)
{
    ...
    debug_msg("[%d]Low power Cmd => 0x%02x\r\n", pktType, uiDCSCmd);
    if(pktType == DSI_SHORT_PACKET)
    {
        if(paramNo >= 2)
        {
            debug_msg("Low power DT param exceed 1 => %d\r\n", paramNo);
            return E_SYS;
        }
...
static ER dispdev_openIFDsi(void)
{
    ...
    UINT32                  uiHFPByteCount;
    UINT32                  uiHBPByteCount;
    ...
    {
        debug_msg("^R Configure pixel format fail\r\n");
    }
#endif
    ...
    if(uiPixelPktModeSel == DSI_PIXMODE_COMMAND_MODE)
    {
        ...
        {
            debug_msg("^R Configure pixel mode command mode fail\r\n");
        }
#endif
...
//          uiVIDEData = (Vtotal - VACT       ) * Htotal                          * DSI_CLK/IDE_CLK
            ...
            //debug_msg("uiHFPByteCount = 0x%08x\r\n", uiHFPByteCount);
            //debug_msg("uiBllPByteCount = 0x%08x LP = %d\r\n", uiBllPByteCount, TLP_TOTAL);
            //debug_msg("uiBHBPByteCount = 0x%08x\r\n", uiHBPByteCount);
        ...
        {
            debug_msg("^R Configure pixel mode video mode fail\r\n");
        }
        else
        {
            debug_msg("^G Configure pixel mode video mode success\r\n");
        }
#endif
