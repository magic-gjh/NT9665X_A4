...
#if _FPGA_EMULATION_
static AUDIO_CLK_SEL        codecClkSel     = AUDIO_CLK_SEL_PLL_DIV;
#else
static AUDIO_CLK_SEL        codecClkSel     = AUDIO_CLK_SEL_CLKMUX;
#endif
...
/**
    Get audio codec function pointer for NT96650 embedded audio codec
...
static void audcodec_setRecordSource(AUDIO_RECSRC RecSrc)
{
    ...
    case AUDIO_RECSRC_AUX:
        ...
        break;
    default:
        debug_err(("%s: unsupported record source: 0x%lx\r\n", __func__, RecSrc));
        break;
    }
}
...
//        pll_setClockFreq(ADOCLK_FREQ, 12288000);
    ...
    {
        dai_setI2SClkRatio(DAI_I2SCLKR_256FS_32BIT);
    }
    else
    {
        dai_setI2SClkRatio(DAI_I2SCLKR_256FS_64BIT);
    }
#else
...
static void audcodec_record(void)
{
    ...
    WM8978CtrlReg.pwm2.uiData |= 0x01F;
    WM8978_sendCommand(&WM8978CtrlReg.pwm2);
...
static BOOL audcodec_setParameter(AUDIO_PARAMETER Parameter, UINT32 uiSetting)
{
    switch(Parameter)
    {
    case AUDIO_PARAMETER_ADO_CLK_SEL:
        codecClkSel = uiSetting;
        break;
...
ER wm8978_writeCtrlReg(UINT32 uiOffset, UINT32 uiValue)
{
    ...
    while (retry--)
    {
        ...
        if (erReturn != E_OK)
        {
            debug_err(("%s: send offset 0x%lx fail\r\n", __func__, uiOffset));
            wm8978_i2cStop();
            Delay_DelayMs(1);
            continue;
        }
        wm8978_i2cWriteByte(uiValue);
...
ER wm8978_readCtrlReg(UINT32 uiOffset, UINT32* puiValue)
{
    ...
    if (erReturn != E_OK)
    {
        debug_err(("%s: send read address fail\r\n", __func__));
        return erReturn;
    }
