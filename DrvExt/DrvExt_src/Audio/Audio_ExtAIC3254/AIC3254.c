...
/**
    Get audio codec function pointer for NT96650 external audio codec
...
static void audcodec_init(PAUDIO_SETTING pAudio)
{
    ...
    case AUDIO_I2SCTRL_I2C:
        ...
        break;
    default:
        debug_err(("%s: unsupported control IF\r\n", __func__, pAudioDeviceObj->uiI2SCtrl));
        break;
    }
...
static void audcodec_setOutput(AUDIO_OUTPUT Output)
{
    ...
    {
    default:
        debug_err(("%s: unsupported play output, force to SPK: 0x%lx\r\n", __func__, Output));
...
//    const UINT32 uiPlayMaxVol = 0x30;
    ...
    else
    {
        ...
        debug_err(("%s: vol 0x%lx, target 0x%lx\r\n", __func__, Vol, uiPlayTargetVol));
...
static void audcodec_setGain(AUDIO_GAIN Gain)
{
    ...
    debug_err(("%s: gain 0x%lx, target 0x%lx\r\n", __func__, Gain, uiRecrdTargetVol));
}
...
static void audcodec_setFormat(AUDIO_I2SFMT I2SFmt)
{
    ...
    {
    default:
        debug_err(("%s: unsupported I2S fmt, force to standard I2S: 0x%lx\r\n", __func__, I2SFmt));
...
static void audcodec_setClockRatio(AUDIO_I2SCLKR I2SCLKRatio)
{
    ...
    {
        debug_err(("%s: clkRatio 0x%lx not support\r\n", __func__, I2SCLKRatio));
    }
}
    ...
    case AUDIO_I2SCTRL_I2C:
        {
            ...
            i2cData.ByteCount = 3;
            i2cData.Byte[0].uiValue = AIC3254_I2C_SADDR;    // slave address of TVL320AIC3254
            i2cData.Byte[0].Param = I2C_BYTE_PARAM_START;
            i2cData.Byte[1].uiValue = uiReg;
            i2cData.Byte[1].Param = I2C_BYTE_PARAM_NONE;
            i2cData.Byte[2].uiValue = uiData;
            i2cData.Byte[2].Param = I2C_BYTE_PARAM_STOP;

            // Lock I2C
            i2c_lock(gI2cSession);

            i2c_transmit(&i2cData);
    ...
    case AUDIO_I2SCTRL_I2C:
        {
            ...
            i2cData.Byte[1].uiValue = uiReg;
            i2cData.Byte[1].Param = I2C_BYTE_PARAM_NONE;
            i2cData.Byte[2].uiValue = AIC3254_I2C_SADDR | 0x01;    // slave address with READ
            i2cData.Byte[2].Param = I2C_BYTE_PARAM_START;
            ...
            do
            {
                ...
                i2cData.ByteCount = 1;
                i2cData.Byte[0].Param = I2C_BYTE_PARAM_STOP | I2C_BYTE_PARAM_NACK;
                i2c_receive(&i2cData);
            } while (0);
            ...
            *puiData = i2cData.Byte[0].uiValue;
        }
        return TRUE;
    default:
        return FALSE;
...
/*
    ...
    @param[in] Parameter    Audio parameter. Available values are below:
     - @b AUDIO_PARAMETER_ADO_CLK_SEL:          Set clock source of HDMI audio

    @param[in] uiSetting    parameter setting of Parameter. Below is the case by case descriptions.
     - @b AUDIO_PARAMETER_ADO_CLK_SEL:          AUDIO_CLK_SEL_CLKMUX: from clock mux. AUDIO_CLK_SEL_PLL_DIV: from clock divider
...
static BOOL audcodec_chkSamplingRate(void)
{
    if (bChkPlySR)
    {
        ...
        {
            debug_err(("%s: installed SR %d, but current SR is %d\r\n",
