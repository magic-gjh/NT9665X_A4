...
INT32 MenuCommonOption_OnReverseGear(VControl *, UINT32, UINT32 *);
...
EVENT_ITEM(NVTEVT_REVERSEGEAR,MenuCommonOption_OnReverseGear)
...
UINT32 TP_PhaseMenuOption_Index(VControl *pCtrl,UINT32 Y)
{
	UINT32  index;
	UINT32 uiOption,uiOptionPerPage,uiTotalOptionOri,uiOptionHigh;
	Ux_RECT Rect,Rect_Panel;

	UxCtrl_GetPos(pCtrl,&Rect);	
	
       uiOptionPerPage = UxMenu_GetData(pCtrl, MNU_PAGEITEM); // items per page
       uiOptionHigh=Rect.y2-Rect.y1;
	 index=(Y-Rect.y1)/uiOptionHigh;
       //index+=(MenuCommonOptionCurrentPage-1)*uiOptionPerPage;
       debug_msg("TP_PhaseMenuOption_Index:%d,uiOptionHigh:%d\r\n",index,uiOptionPerPage);
...
INT32 MenuCommonOption_OnReverseGear(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    Ux_CloseWindow(&MenuCommonItemCtrl,0); // close whole tab menu
    return NVTEVT_CONSUME;
}
...
INT32 MenuCommonOption_Menu_OnTimer(VControl *, UINT32, UINT32 *);
...
#endif
EVENT_ITEM(NVTEVT_TOUCHPANEL_KEY,MenuCommonOption_Menu_OnTouchPanelKey)
EVENT_ITEM(NVTEVT_TIMER,MenuCommonOption_Menu_OnTimer)
...
INT32 MenuCommonOption_Menu_OnTouchPanelKey(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	...
	P2=paramArray[1];  
	    ...
	    if(TPIsOnRange(&MenuCommonOption_Button_ReturnCtrl,P1,P2)==TRUE)
	    	{
			Ux_CloseWindow(&MenuCommonOptionCtrl,0); // close whole tab menu
	    	}				
	    else if(TPIsOnRangeMenu(&MenuCommonOption_MenuCtrl,P1,P2)==TRUE)
		{
			MenuCommonOptionIndex = TP_PhaseMenuOption_Index(&MenuCommonOption_MenuCtrl,P2);
			debug_msg("MenuOption press index:%d,pItem->Count:%d..\r\n",MenuCommonOptionIndex,pItem->Count);
				
		      if ((MenuCommonOptionIndex < 0) || (MenuCommonOptionIndex > pItem->Count-1))
		      {
		        	MenuCommonOptionIndex = 0;
		      }		
			else
			{
		              MenuCommonOptionTPKeyHasPressed=TRUE;			
				UxMenu_SetData(&MenuCommonOption_MenuCtrl, MNU_CURITM, MenuCommonOptionIndex);
		                if (g_uiTouchKeyTimerID == NULL_TIMER)
		                {
		                    g_uiTouchKeyTimerID = GxTimer_StartTimer(100, NVTEVT_01SEC_TIMER, ONE_SHOT);
		                }					
			}
                       // MenuCommonOption_ScrollBar_setstep(MenuCommonOptionIndex);
		}				
	return NVTEVT_CONSUME;
}


INT32 MenuCommonOption_Menu_OnTimer(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    NVTEVT event;
    TM_MENU*    pMenu = TM_GetMenu();
    ...
    switch(event)
    {
        case NVTEVT_01SEC_TIMER: 
	     if(MenuCommonOptionTPKeyHasPressed==TRUE)
	     {
      	            MenuCommonOptionTPKeyHasPressed=FALSE;
	            if (g_uiTouchKeyTimerID != NULL_TIMER)
	            {
			...
			UxMenu_SetData(&MenuCommonOption_MenuCtrl, MNU_CURITM, MenuCommonOptionIndex);				      	
			return MenuCommonOption_Menu_OnKeyEnter(&MenuCommonOption_MenuCtrl, paramNum, paramArray);    	     
	     }
        break;
    }
    return NVTEVT_CONSUME;
