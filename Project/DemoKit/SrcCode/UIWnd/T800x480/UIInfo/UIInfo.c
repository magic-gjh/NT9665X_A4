...
void SysSetFixedFlag(void)
{
    ...
    //SysSetFlag(FL_MOVIE_SIZE_DUAL, 	 DEFAULT_MOVIE_SIZE_DUAL);
    ...
    SysSetFlag(FL_CAR_NUM,	CAR_NUM_OFF);		
...
BOOL TPIsOnRangeMenu(VControl *pCtrl,UINT32 P1,UINT32 P2)
{
 	Ux_RECT Rect;
	UxMenu_GetRange(pCtrl,&Rect);
	debug_msg("Menu:x1:%d,x2:%d,y1:%d,y2:%d,p1:%d,p2:%d...\r\n",Rect.x1,Rect.x2,Rect.y1,Rect.y2,P1,P2);
	//if(Rect.x1>CALIBRATION_X_RANG)
	//Rect.x1-= CALIBRATION_X_RANG;		
...
BOOL TPIsOnRange(VControl *pCtrl,UINT32 P1,UINT32 P2)
{
 	Ux_RECT Rect;
...
BOOL TPIsOnRangeList(VControl *pCtrl,UINT32 P1,UINT32 P2)
{
 	Ux_RECT Rect;
	UxList_GetRange(pCtrl,&Rect);
	debug_msg("List:x1:%d,x2:%d,y1:%d,y2:%d,p1:%d,p2:%d...\r\n",Rect.x1,Rect.x2,Rect.y1,Rect.y2,P1,P2);
...
UINT32 TP_PhaseListItem_Index(VControl *pCtrl,UINT32 P1)
{
	UINT32  index;
	Ux_RECT Rect;

	UxCtrl_GetPos(pCtrl,&Rect);	
	debug_msg("TP_PhaseListItem_Index:%d,%d,%d,%d..\r\n",Rect.x1,Rect.x2,Rect.y1,Rect.y2);
       if((P1>Rect.x1)&&((P1<Rect.x2)))
       {
		index=0;		
	 }
	 else
