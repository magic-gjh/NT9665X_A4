...
static BOOL FlowPB_ThumbConfig(THUMBNAIL_CONFIG *pThumbCfg,USIZE size)
{
    UINT32  i;
...
PURECT FlowPB_ThumbDisplay(UINT32 Thumbdevice,USIZE size)
{
    THUMBNAIL_CONFIG ThumbCfg;


        if (Thumbdevice == PLB_THUMB_LCD)
        {
            #if (FLOWPB_THUMB_MODE == FLOWPB_THUMB_MODE_3_3)
            if (g_bDualUI==TRUE&&SysGetFlag(FL_DualDisp) == DUALDISP_ONXOR)
            {
                ThumbCfg.wFirstHGap         = 80;
                ThumbCfg.wHGap              = 80;
                ThumbCfg.wLastHGap          = 80;
        ...
        FlowPB_ThumbConfig(&ThumbCfg,size);
//#NT#2012/8/13#Philex - begin
//mark code
#if 0
        FlowPB_ShowThumbBG();
