...
typedef struct _TM_MENU
{
    ...
    INT16           SelPage;        // current Page (start with 0)
    INT16           FirstPage;      // 1st Page on current display region
...
#define TMDEF_OPTION_TEXT_ICON(id)      { IDS_##id, IDI_##id},

// Option has text only (text will align to left)
#define TMDEF_OPTION_TEXT(id)           { IDS_##id, 0 },

// Option has icon only (icon will align to right)
#define TMDEF_OPTION_ICON(id)           { 0, IDI_##id },
...
// normal Item
#define TMDEF_ITEM_TEXTID(name)         { IDM_##name, IDS_##name, IDI_##name, FL_##name,\
