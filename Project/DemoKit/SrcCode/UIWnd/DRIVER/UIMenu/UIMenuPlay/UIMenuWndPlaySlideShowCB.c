...
EVENT_ITEM(NVTEVT_KEY_CUSTOM1,UIMenuWndPlaySlideShowCB_OnKeyShutter2)
#else
...
INT32 UIMenuWndPlaySlideShowCB_OnOpen(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    if (AppPlay_GetData(PLAY_FILENUM) > 0)
    {
#if SLIDESHOW_JPEG_ONLY
        // play next if current file format is video
        while (AppPlay_GetData(PLAY_FILEFMT)&PBFMT_AVI||AppPlay_GetData(PLAY_FILEFMT)&PBFMT_MOVMJPG)
        {
           Ux_SendEvent(&UIPlayObjCtrl,NVTEVT_PLAYSINGLE, 2, PB_SINGLE_NEXT, 1);
        }
        if (AppPlay_GetData(PLAY_FILEFMT)&PBFMT_JPG)
#endif
        {
