...
INT32 UIFlowWndPlayMagnify_OnOpen(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    if (AppPlay_GetData(PLAY_IMGWIDTH_ORI) < 1280/*JPGIMG_SIZE[_IMGSize_1M][0]*/)
    {
    ...
    g_uiZoomIndex   = AppPlay_GetData(PLAY_ZOOMINDEX);
    g_uiOriImageWidth  = AppPlay_GetData(PLAY_IMGWIDTH);
    g_uiOriImageHeight = AppPlay_GetData(PLAY_IMGHEIGHT);
...
INT32 UIFlowWndPlayMagnify_OnKeyZoomout(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    if(g_uiZoomIndex > 1)
    {
        Ux_SendEvent(&UIPlayObjCtrl,NVTEVT_ZOOMINOUT, 1, PLAYZOOM_OUT);
        g_uiZoomIndex   = AppPlay_GetData(PLAY_ZOOMINDEX);
        UIFlowWndPlayMagnify_DrawNavigatorWindow();
    }
    else
...
static void UIFlowWndPlayMagnify_DrawNavigatorWindow(void)
{
    ...
    uiPanSrcStartX = AppPlay_GetData(PLAY_PANSRCX);
    uiPanSrcStartY = AppPlay_GetData(PLAY_PANSRCY);
    uiPanMaxX = AppPlay_GetData(PLAY_PANMAXX);
    uiPanMaxY = AppPlay_GetData(PLAY_PANMAXY);
...
static INT32 UIFlowWndPlayMagnify_ZoomNavi(UINT32 key)
{
    ...
    ZoomIndex   = AppPlay_GetData(PLAY_ZOOMINDEX);
    usImagewidth  = AppPlay_GetData(PLAY_IMGWIDTH);
    usImageheight = AppPlay_GetData(PLAY_IMGHEIGHT);
    ...
    Ux_SendEvent(&UIPlayObjCtrl,NVTEVT_ZOOMNAVI,3, SetPlayCommand, MoveOffset_X,MoveOffset_Y);
    UIFlowWndPlayMagnify_DrawNavigatorWindow();
    return NVTEVT_CONSUME;
}
