...
void AppInit_ModeUSBSIDC(void)
{
    ...
    else if (strcmp(pDxName,"Storage_Card1")==0)
    {
        ...
        //#NT#Modify for dual card
        //#NT#2010/07/20#Jeah Yen -begin
        SIDCInfo.uiStrgCardLock = GxStrg_GetDeviceCtrl((UINT32)GxStrg_GetDevice(0), CARD_READONLY);
...
INT32 PrintExe_OnOpen(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    AppInit_ModeUSBSIDC();
    AppPrint_RegSetPlayObjCB(PrintExe_SetPlayInfo);
...
CREATE_APP(CustomPrintObj,APP_PRINT)
