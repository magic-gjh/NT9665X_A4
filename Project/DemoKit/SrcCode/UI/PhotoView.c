...
void PhotoDisplay2_Update(UINT8 iDD, ISIZE *pSize)
{
    ...
    #else
    {
        ...
        UIDebug_Show(("pSize w=%d,h=%d\r\n",pSize->w,pSize->h));
		UIDebug_Show(("dev1Ratio w=%d,h=%d\r\n",dev1Ratio.w,dev1Ratio.h));
    ...
    SWAP(UINT32,win.x,win.y);//jacky lan
    // swap buffer w/h
    ...
    //#NT#Graphic rtoate is 16-bytes alignment,it should use line offset.
    Img.buf_LineOffset = DispOutImg.LineOffset[0];
    //#NT#2015/03/27#KS Hung -end
    // force format to 420_PK
    Img.buf_Format = PXLFMT_YUV420_PK;
    #endif
    UIDebug_Show(("\n\r"));
    UIDebug_Show(("^MImg w=%d,h=%d,format:%x\r\n",Img.buf_w,Img.buf_h,Img.buf_Format));
    UIDebug_Show(("^Mwin x=%d,y=%d,w=%d,h=%d\r\n",win.x,win.y,win.w,win.h));
