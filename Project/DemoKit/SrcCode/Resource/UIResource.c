...
PF_PREPARE_USER_WP pfPrepareUserWP = 0;
...
void Reg_UserWPFunc(PF_PREPARE_USER_WP fp)
{
    pfPrepareUserWP = fp;
}

//#NT#2010/02/26#Jeah Yen -begin
const IMAGE* Get_LOGOImage(void)
{
    debug_err(("GET GE-LOGO\r\n"));
    if(!p_gDefLogoJPG)
    {
        IMAGE_LoadFromJPG(&gDefLogoImage, (UINT32)g_ucBGOpening, Logo_getBGOpening_size());
        p_gDefLogoJPG = (UINT32)&gDefLogoImage;
    }
    return (const IMAGE*)&gDefLogoImage;
}
//#NT#2010/02/26#Jeah Yen -end

const IMAGE* Get_WPImage(UINT32 id)
{
    //#JPG using Pstore
    if(id == WP_MENU) //menu wallpaper
    {
        if(!p_gMenuJPG)
            return 0;
        return &gMenuImage;
    ...
    //#NT#Fix compile warnings
    else //if(id == WP_LOGO) //wellcome wallpaper
    {
        debug_err(("GET USER-LOGO\r\n"));
        if(!p_gLogoJPG)
            return 0;
        return &gLogoImage;
    }
...
BOOL UIRes_ChkWriteWP(UINT32 id)
{
    ...
    //Store Image to PStore
    if(id == WP_MENU) //menu wallpaper
    {
        p_gMenuJPG = 0;
        p_gMenuSize = 0;
        memset(&gMenuImage, 0, sizeof(IMAGE));

        if(!pfPrepareUserWP)
        {
            debug_err(("\n[WP] PStore Prepare WP[%d] fail\r\n", id));
            return FALSE;
        }

        //#NT#2010/03/08#Jeah Yen -begin
        //call user wallpaper prepare func
        tmpSize = (*pfPrepareUserWP)(tempBuf, POOL_SIZE_GFX_IMAGE);
        //#NT#2010/03/08#Jeah Yen -end

        pSection = PStore_OpenSection(PS_WALLPAPER_IMAGE, PS_RDWR | PS_CREATE);
        if(pSection == E_PS_SECHDLER)
        {
            debug_err(("\n[WP] PStore Write WP[%d] fail\r\n", id));
            return FALSE;
        }
        //Save File Size (4Bytes)
        PStore_WriteSection((UINT8*)&tmpSize,0,4,pSection);
        //Save JPG
        PStore_WriteSection((UINT8*)tempBuf,4,tmpSize,pSection);

        PStore_CloseSection(pSection);
        p_gMenuJPG = tempBuf;
        p_gMenuSize = tmpSize;
        IMAGE_LoadFromJPG(&gMenuImage, p_gMenuJPG, p_gMenuSize);
        UIRes_Debug(("\n[WP] PStore Write WP[%d] size = %d\r\n", id, tmpSize));
    }
    else if(id == WP_LOGO) //wellcome wallpaper
    {
        p_gLogoJPG = 0;
        p_gLogoSize = 0;
        memset(&gLogoImage, 0, sizeof(IMAGE));
        ...
        pSection = PStore_OpenSection(PS_LOGO_IMAGE, PS_RDWR | PS_CREATE);
        if(pSection == E_PS_SECHDLER)
        {
            debug_err(("\n[WP] PStore Write WP[%d] fail\r\n", id));
            return FALSE;
        ...
        p_gLogoJPG = tempBuf;
        p_gLogoSize = tmpSize;
        IMAGE_LoadFromJPG(&gLogoImage, p_gLogoJPG, p_gLogoSize);
        UIRes_Debug(("\n[WP] PStore Write WP[%d] size = %d\r\n", id, tmpSize));
    }
...
void UIRes_InitReadWP(UINT32 id)
{
    UINT32 tempBuf, tmpSize;
    PPSTORE_SECTION_HANDLE  pSection;
    ...
    if(id == WP_MENU) //menu wallpaper
    {
        ...
        pSection = PStore_OpenSection(PS_WALLPAPER_IMAGE, PS_RDWR);
        if(pSection == E_PS_SECHDLER)
        {
...
void UIRes_DeleteWP(UINT32 id)
{
    if(id == WP_MENU) //menu wallpaper
    {
        PStore_DeleteSection(PS_WALLPAPER_IMAGE);
        p_gMenuJPG = 0;
        p_gMenuSize = 0;
        memset(&gMenuImage, 0, sizeof(IMAGE));
        UIRes_Debug(("[WP] delete WP[%d] ok\r\n", id));
    }
    else if(id == WP_LOGO) //wellcome wallpaper
    {
        PStore_DeleteSection(PS_LOGO_IMAGE);
        p_gLogoJPG = 0;
        p_gLogoSize = 0;
        memset(&gLogoImage, 0, sizeof(IMAGE));
        UIRes_Debug(("[WP] delete WP[%d] ok\r\n", id));
...
void UIRes_InitReadPHFE(void)
{
    ...
    {
        PStore_ReadSection((UINT8 *)&gPFInfo, 0, sizeof(gPFInfo), pSection);
        PStore_ReadSection((UINT8 *)&tmp, gPFInfo.TotalLen-2, 2, pSection);
        if (gPFInfo.Ver == PF_PSTORE_VER &&  tmp == 0xD9FF)
        {
...
{
    PStore_DeleteSection(PS_PHOTOFRAME_IMAGE);
    UIRes_Debug(("[PF] delete ok\r\n"));
}
...
BOOL UIRes_ChkWriteSSWAV(void)
{
    ...
    for(cnt = 0; cnt < SSWAV_NUM; cnt ++)
    {
        ...
        ret = FileSys_ReadFile(filehdl,(UINT8*)uiWAVBuffer,&filesize, 0, 0);
        FileSys_CloseFile(filehdl);
        if(filesize != 0)
        {
            bIsUpdate = TRUE;
...
void UIRes_InitReadSSWAV(int SSWAV_id)
{
    ...
    if ((pSection = PStore_OpenSection(PS_SSWAV_SIZ, PS_RDWR)) != E_PS_SECHDLER)
    {
        PStore_ReadSection((UINT8 *)g_pSSWAVSize_, 0, sizeof(g_pSSWAVSize_), pSection);
        PStore_CloseSection(pSection);
    }
    else
    ...
    if ((pSection = PStore_OpenSection(g_SSWAVInfo[SSWAV_id].uiPS_SectionName, PS_RDWR)) != E_PS_SECHDLER)
    {
       ...
       //#NT#2011/02/16#Steven feng -end
        ...
        PStore_ReadSection((UINT8 *)uiWAVBuffer, 0, g_pSSWAVSize_[SSWAV_id], pSection);
        PStore_CloseSection(pSection);
...
//#Multi lang using Pstore
    ...
    UI_SetData(FL_LanguageIndex,lang_id);
