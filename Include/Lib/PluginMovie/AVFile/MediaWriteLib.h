...
#define MEDIAREC_H264GOP_IPONLY     0 // Hideo@120322 ///< H264 IP only
#define MEDIAREC_H264GOP_IPB        1 // Hideo@120322 ///< H264 IPB
...
typedef enum
{
    ...
    MEDIAWRITE_SETINFO_H264GOPTYPE      = 0x8C, ///< mov: p1:GOP type, MEDIAREC_H264GOP_IPONLY or others (in), p3:id(in)
...
/**
    Callback function type for media recording. Event types are defined in MediaRecAPI.h.
*/
typedef void (RawEncCallbackType)(UINT32 uiEventID);


/**
    Callback function type for RawEncode
*/
typedef INT32 (*FSWRITECB)(UINT32 Addr, UINT32 Size, UINT32 Fmt);
...
typedef struct
{
    ...
    MEDIAREC_VIDENC_BSTR_INFO   BStrInfo;   ///< input video bitstream test pattern info
    MEDIAREC_VIDENC_YUV_INFO    YUVInfo;    ///< fixed YUV test pattern information
} MEDIAREC_TESTMODE;
...
typedef struct
{
    ...
    UINT32              uiFileType;         ///< file type, MEDIAREC_AVI, MEDIAREC_MOV, MEDIAREC_MP4
    UINT32              uiSeamlessSec;      ///< seamless cut second,
    UINT32              uiEndType;          ///< end type, MEDIAREC_ENDTYPE_NORMAL or others
    BOOL                bCustomUdata;       ///< customized user data mode (no default thumbnail, all user data prepared by customer)
    BOOL                bPwrOffProtect;     ///< enable power-off protection, default: off
    BOOL                bGPSon;             ///< enable GPS info, default: off

    //video setting
    UINT32              uiVidCodec[VIDENC_ID_MAX];  ///< recording video codec, MEDIAREC_ENC_JPG, MEDIAREC_ENC_H264
    UINT32              uiWidth;            ///< recording video width
    UINT32              uiHeight;           ///< recording video height
    UINT32              uiWidth2;           ///< recording video width
    UINT32              uiHeight2;          ///< recording video height
    UINT32              uiVidFrameRate[VIDENC_ID_MAX];  ///< recording video frame rate, MEDIAREC_VRATE_30
    BOOL                bEnableBRCtrl;      ///< enable rate control or not
    UINT32              uiTargetBitrate;    ///< video data bytes per second
    UINT32              uiTargetBitrate2;   ///< video data bytes per second
    UINT32              uiH264GopType;      ///< H264 GOP type (I/P only, I/P/B, ...)
    UINT32              ui3DNRLevel;        ///< H.264 3DNR level
    UINT32              uiQPLevel;          ///< H.264 QP initial level
    UINT32              uiDropFrameNum;     ///< drop frame number as start recording (for AV sync)
    ...
    UINT32              uiJpgYuvFormat;     ///< jpeg yuv format setting
    //#NT#2010/09/17#Calvin Chang -end
    VIDENC_DAR          uiDAR[VIDENC_ID_MAX];   ///< video display aspect ratio
    BOOL                bPreEncode;         ///< pre-encode (for applying 3DNR to 1st frame)

    //audio setting
    UINT32              uiAudCodec;         ///< recording audio codec, MEDIAREC_ENC_PCM, MEDIAREC_ENC_PCM
    ...
    UINT32              audioCh;            ///< audio channel type, AUDIO_CH_LEFT, AUDIO_CH_RIGHT, AUDIO_CH_STEREO, ...
    BOOL                bAudFilterEn;       ///< enable/disable audio filter (set TRUE to enable)

    //raw enc setting
    UINT32              uiEncQ;             ///< input encoding Q //2010/01/04 Meg

    UINT32              uiSensorFPS;        ///< input sensor fps (30/60/120/240)//2014/06/10 Meg
    BOOL                bTinyModeOn;        ///< input tiny mode //2014/07/17 Calvin Chang
...
typedef struct
{
    ...
    UINT32              uiRotationInfo;     ///< rotation information in Mov/Mp4 file
    //UINT32              uiFre1Addr;         ///< [MOV] user data addr for MOV
    //UINT32              uiFre1Size;         ///< [MOV] user data size for MOV
...
typedef struct
{
    ...
    ER (*cbReadFile)(UINT8 *type, UINT32 pos, UINT32 size, UINT32 addr);  ///< a file-reading function for fileformat maker
...
#define MEDIANR_CUSTOM_SET_FILESIZE     0x05 ///< p1: CHAR *pPath (in), p2: UINT32 filesize (in)
#define MEDIANR_CUSTOM_GET_FILESIZE     0x06 ///< p1: CHAR *pPath (in), p2: UINT32 *filesize (out)
