...
/**
    Register callback function for getting current audio PCM address/size

    Register callback function for getting current audio PCM address/size

    @param[in] fpGetAudPCMCB    AUDFILTER_FILTER_CB callback function pointer

    @return void
*/
extern void AudFilterTsk_RegGetAudPCMCB(AUDFILTER_FILTER_CB fpAudGetPCMCB);

/**
    Get Audio PCM Current Address

    Get Audio PCM Current Address

    @return The audio process data address and size.
*/
extern void AudFilter_GetCurrAudPCMAddr(UINT32 *puiAddr, UINT32 *puiSize);

/**
    Set Audio PCM Current Address

    Set Audio PCM Current Address

    @param[in] uiAddr    The audio data address.
    @param[in] uiSize    The audio data size.

    @return void
*/
extern void AudFilter_SetCurrAudPCMAddr(UINT32 uiAddr, UINT32 uiSize);
