...
*/
typedef struct _PBXFile_Access_Info
{
    PBX_FILE_CMD  fileCmd;          ///< File operation command
    UINT8*        fileName;         ///< File name, should be full path, ex: "A:\DCIM\100NVTIM\IMAG0001.JPG"
    UINT8*        pBuf;             ///< File R/W buffer
    UINT32        bufSize;          ///< File R/W buffer size
    UINT32        filePos;          ///< File R/W position
}PBXFile_Access_Info, *PPBXFile_Access_Info;
...
/**
     ...
     @note the file access will firstly open the file, then seek to the file position to do the fileCmd, finally will close the file.
...
*/
extern ER     PBXFile_Access(PPBXFile_Access_Info  pfileAccess);
