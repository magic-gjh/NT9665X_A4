...
#define PBX_FLIST_FILE_TYPE_ANYFORMAT           (0x03FF)  ///< any file format means JPG|AVI|WAV|RAW|TIF|MPO|MOV|MP4|MPG|ASF
//@}
...
typedef enum _PBX_FLIST_CONFIG_ID
{
    ...
    PBX_FLIST_CONFIG_RECURSIVE                = 0x0C,    ///< only for FileDB, set if recursive scan sub folders
...
typedef enum _PBX_FLIST_GETINFO_ID
{
    ...
    PBX_FLIST_GETINFO_FILESIZE         = 0x44,    ///< File size
    PBX_FLIST_GETINFO_FILETYPE         = 0x45,    ///< File type
    PBX_FLIST_GETINFO_FILEPATH         = 0x46,    ///< File path
...
typedef struct
{
    ...
    ER (*GetInfo)(UINT32  InfoID, UINT32 *pparam1, UINT32 *pparam2); ///< Get some info of file list
