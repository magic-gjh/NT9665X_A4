...
#define FWSRV_API_VERSION 0x12010317U   ///< API version
#define FWSRV_PL_BURST_END_TAG  0xFFFFFFFFU ///< indicate end number for FWSRV_PL_LOAD_BURST_IN::puiIdxSequence

/**
     @name FwSrv Utility Macro
*/
//@{
/**
    Calcuate the address of firmware start in memory with an offset bacause the
    firmware begin in memory is not from the address 0.
*/
#define FWSRV_GET_MEM_BEGIN_OFFSET(x)  (OS_GetMemAddr(MEM_CODE)+(x))
//@}
...
typedef enum _FWSRV_CMD_IDX{
    ...
    FWSRV_CMD_IDX_PL_LOAD_SINGLE,   ///< Input:FWSRV_PL_LOAD_SINGLE_IN
    FWSRV_CMD_IDX_PL_LOAD_BURST,    ///< Input:FWSRV_PL_LOAD_BURST_IN
    FWSRV_CMD_IDX_FW_WRITE,         ///< Input:FWSRV_BUF_DESC
    FWSRV_CMD_IDX_FW_READ,          ///< Input:FWSRV_BUF_DESC
    FWSRV_CMD_IDX_LD_WRITE,         ///< Input:FWSRV_BUF_DESC
    FWSRV_CMD_IDX_LD_READ,          ///< Input:FWSRV_BUF_DESC
    FWSRV_CMD_IDX_BLK_WRITE,        ///< Input:FWSRV_BLK_DESC ,for advance write, write block directly.
    FWSRV_CMD_IDX_BLK_READ,         ///< Input:FWSRV_BLK_DESC ,for advance read, read block directly.
    FWSRV_CMD_IDX_GET_BLK_SIZE,     ///< Input:NULL, Output:UINT32 ,get block size by byte
    FWSRV_CMD_IDX_BIN_DEPACK,       ///< Input:FWSRV_BIN_DEPACK, de-pack an all in one bin file to get each resource item
...
/**
    ...
    There are two area in firmware area, one is loader area, the other is main code area.
*/
typedef enum _FWSRV_AREA{
    FWSRV_AREA_UNKNOWN  = 0,        ///< unknown area
    FWSRV_AREA_LOADER,              ///< loader area
    FWSRV_AREA_FIRMWARE,            ///< firmware(main code) area.
    FWSRV_AREA_FIRMWARE_MAX_NUM,    ///< total area numbers
    ENUM_DUMMY4WORD(FWSRV_AREA)
}FWSRV_AREA;

typedef void (*FWSRV_FP_STATE_CB)(FWSRV_STATE State); ///< State callback. If there's any state changed, the service will callback out.
typedef void (*FWSRV_FP_ERROR_CB)(FWSRV_ER er); ///< Error callback. If there's any error occured, the service will callback out.
...
typedef void (*FWSRV_CB_FINISH)(const FWSRV_FINISH* pInfo); ///< Command finish callback. When any command is finished, the service will callback out.
/**
    Command Property.
...
/**
     ...
     For command FWSRV_CMD_IDX_PL_LOAD_SINGLE, load an given index part to memory
*/
typedef struct _FWSRV_PL_LOAD_SINGLE_IN{
    UINT32 uiIdxRegion;       ///< region index from 0 to n
}FWSRV_PL_LOAD_SINGLE_IN,*PFWSRV_PL_LOAD_SINGLE_IN;
...
*/
typedef struct _FWSRV_BLK_DESC{
    FWSRV_AREA Area;    ///< area Type
    UINT32     uiIdx;   ///< logical block index
    FWSRV_BUF_DESC Buf; ///< buffer description
}FWSRV_BLK_DESC,*PFWSRV_BLK_DESC;
...
typedef BOOL (*FWSRV_BIN_DEPACK_CB)(FWSRV_BIN_ITEM* pItem); ///< call back prototype for getting item

/**
     Depack input structure

     For de-packing the all-in-one bin file, the source file path, a working buffer
     , and a callback getting item infomation are requirement.
*/
typedef struct _FWSRV_BIN_DEPACK{
    char* pPath;            ///< source bin file path. eg. "A:\\FW96650A.bin"
    UINT32 uiWorkBufAddr;   ///< working buffer address
    UINT32 uiWorkBufSize;   ///< working buffer size, the requirement formula is (item max size + 160 bytes)
    FWSRV_BIN_DEPACK_CB fpItemCb; ///< callback for get each item information and context.
}FWSRV_BIN_DEPACK,*PFWSRV_BIN_DEPACK;
